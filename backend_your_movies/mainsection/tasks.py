...
def convert_video(source, instance_pk, resolution):
    ...
    cmd = ["ffmpeg", "-i", f"{source}", "-s", f"{resolutions[resolution]}", "-c:v", "libx264", "-crf", "23", "-c:a", "aac", "-strict", "-2", f"{target}"]
    subprocess.run(cmd)
...
def generate_thumbnail(source, thumbnail_name):
    ...
    while os.path.exists(target):
        i += 1
        target = os.path.join(settings.MEDIA_ROOT, "thumbnails", f"{thumbnail_name}{i:03d}.jpg")
    cmd = ["ffmpeg", "-ss", "0", "-i", f"{source}", "-frames:v", "1", "-q:v", "2", "-update", "1", f"{target}"]
    subprocess.run(cmd)
    return target
